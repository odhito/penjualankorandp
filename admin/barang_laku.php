...
<h3><span class="glyphicon glyphicon-briefcase"></span>  Data Koran Terjual</h3>
...
<table class="table">
	...
	?>
	<tr>
		<td colspan="7">Total Laba Kotor</td>
		<?php 
		if(isset($_GET['tanggal'])){
			$tanggal=mysql_real_escape_string($_GET['tanggal']);
	...
	<tr>
		<td colspan="7">Total Laba Bersih</td>
		<?php 
		if(isset($_GET['tanggal'])){
			$tanggal=mysql_real_escape_string($_GET['tanggal']);
				...
				<div class="modal-body">				
					<form action="barang_laku_act.php" method="post">
						...
						<div class="form-group">
							...
							<?php 
								$harga=mysql_query("select * from barang where harga");
								while($h=mysql_fetch_array($harga)){
									?>	
									<option value="<?php echo $h['harga']; ?>"><?php echo $h['harga'] ?></option>
									<?php 
								}
								?>
							</select>
